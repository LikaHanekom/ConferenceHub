...
export default async function RoomsPage({
  ...
  searchParams: Promise<{ floor?: string; capacity?: string; available?: string }>;
}) {
  const { floor, capacity, available } = await searchParams;
  ...
  if (floor) {
    rooms = rooms.filter((r) =>
      r.floor.toLowerCase().includes(floor.toLowerCase())
    );
  }
    ...
    <main className="min-h-screen bg-gray-50 p-8 dark:bg-gray-900">
      <div className="mx-auto max-w-5xl">
        ...
        <p className="mb-6 text-sm text-gray-500 dark:text-gray-400">
          {rooms.length} of {allRooms.length} rooms shown
        </p>
